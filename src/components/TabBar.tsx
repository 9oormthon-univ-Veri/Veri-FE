...
interface TabItem {
  id: string;
  name: string;
  icon: React.ComponentType<{ active: boolean }>; // 아이콘 컴포넌트를 직접 받도록 타입 변경
  path: string;
  isDummy?: boolean;
}
...
const currentTabs: TabItem[] = [
  ...
  { id: 'camera', name: '', icon: () => null, path: '/camera', isDummy: true }, // 더미 탭으로 카메라 아이콘은 제외
    ...
    <div className="tab-bar-container">
      {currentTabs.map((tab) => {
        ...
        return (
          ...
          >
            {/* 더미 탭이 아닐 때만 아이콘과 텍스트를 렌더링 */}
            {!tab.isDummy && (
              <>
                <div className="tab-item-icon">
                  {/* 아이콘 컴포넌트를 렌더링하고 active prop 전달 */}
